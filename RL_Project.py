...
Original file is located at
    https://colab.research.google.com/drive/1gSAJoMt7XwF0L7Tj3sNZ7lw4HN25Rf17
"""
...
def train(iot_RL_list, NUM_EPISODE):
    ...
    for episode in range(NUM_EPISODE):
        ...
        #No. of tasks that arrived in given episode
        tasks_ep = 0
        for row in bitarrive:
          for col in row:
            if col != 0:
              tasks_ep = tasks_ep + 1
        ...
        unfinish_indi = env.process_delay_unfinish_ind
        tasks_drop_ep = 0
        for row in unfinish_indi:
          for col in row:
            if col != 0:
              tasks_drop_ep = tasks_drop_ep + 1

        Total_dropped.append(tasks_drop_ep)
        print(f'Total tasks in episode {episode}: {tasks_ep}')
        print(f'Total tasks dropped in episode {episode}: {tasks_drop_ep}')
...
if __name__ == "__main__":
    ...
    # TRAIN THE SYSTEM
    Total_tasks, Total_dropped = train(iot_RL_list, NUM_EPISODE)
...
avg_rewards = list()
for episode in range(NUM_EPISODE):
  avg_rewards.append(np.mean(episode_rewards[episode]))
...
plt.xlabel('Episodes')
plt.ylabel('Rewards')
plt.title('Average Rewards v/s Episodes')
...
avg_delays = list()
for episode in range(NUM_EPISODE):
  avg_delays.append(np.mean(episode_delays[episode]))

avg_delays

len(avg_delays)

plt.plot(range(len(avg_delays)), avg_delays)

plt.xlabel('Episode')
plt.ylabel('Average Delay')
plt.title('Average Delay v/s Episodes')

plt.show()

print(len(Total_tasks), len(Total_dropped))

Total_dropped = [float(i) for i in Total_dropped]
Total_tasks = [float(i) for i in Total_tasks]
drop_ratio = [Total_dropped[i] / Total_tasks[i] for i in range(len(Total_dropped))]

plt.plot(range(len(drop_ratio)), drop_ratio)

plt.xlabel('Episode')
plt.ylabel('Ratio of Dropped Tasks')
plt.title('Task drop ratio v/s Episodes')
